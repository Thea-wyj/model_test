...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if (epoch + 1) == EPOCHS:  # 只画出最后一个epoch时候的准确度变化曲线
            ...
            train_accuracy_list.append(accuracy.cpu().numpy())
...
def val(model, device, test_loader, epoch):
    ...
    with torch.no_grad():
        ...
        test_accuracy_list.append(acc.cpu().numpy())
...
torch.save(resnet_model, 'pretrainedModel.pth')  # 保存模型

# 可视化测试机的loss和accuracy
plt.figure(1)
plt.plot(test_iteration_list, test_loss_list)
plt.title("ResNet34 test loss")
plt.ylabel("loss")
